...
(function() {
    const fileNameSuffix = "--cvwebrequestsvanilla";
    const portName = "cvwebrequests";
    const urlCheck = {}
    const cvwebrequestsLog = {}
    const tabStorage = {};
            ...
            if(changeInfo.status == 'loading' && (urlCheck[tabId].url != tab.url) ) {
                ...
                if(urlCheck[tabId].url != '') {
                    ...
                    var fileData = {
                        ...
                        cvwebrequests: cvwebrequestsLog[tabId]
                    }

                    if (fileData.cvwebrequests.length > 0) {
                        // take the first event as the starttime of the original url
                        var firstWebRequest = fileData.cvwebrequests[0];
                        fileData.startTime = firstWebRequest.time;
                    } else {
                        fileData.startTime = "";
                    }
                ...
                logEvent('onTabNewURL', {"url": tab.url, "tabId": tabId}, tabId);
                ...
                logEvent('onTabComplete', {"url": tabUrl, "tabId": tabId}, tabId);
    ...
    chrome.tabs.onActivated.addListener((tab) => {
        ...
        //checkTabStorage(tabId);
        logEvent('onTabActivated', {"tabId": tabId}, tabId);
    });
    ...
    chrome.tabs.onRemoved.addListener((tab) => {
        ...
        logEvent('onTabRemoved', {"tabId": tabId}, tabId);
    });

    function logEvent(eventName, eventData, tabId) {

        // if log does not have tabId yet
        if (!cvwebrequestsLog.hasOwnProperty(tabId)) {
    ...
    chrome.webRequest.onSendHeaders.addListener((details) => {
        ...
        logEvent('onSendHeaders', eventData, tabId);
    ...
    chrome.webRequest.onBeforeRedirect.addListener((details) => {
        ...
        var eventData = {
            ...
            url: details.url,
            requestTime: details.timeStamp,
            status: 'onBeforeRedirect',
            details: JSON.stringify(details)
        };
        console.log(eventData);
        logEvent('onBeforeRedirect', eventData, tabId);
    ...
    chrome.webRequest.onCompleted.addListener((details) => {
        ...
        console.log(eventData);
        logEvent('onCompleted', eventData, tabId);
    ...
    chrome.webRequest.onErrorOccurred.addListener((details)=> {
        ...
        var eventData = {
            ...
            url: details.url,
            requestTime: details.timeStamp,
            status: 'onErrorOccurred',
            details: JSON.stringify(details)
        };

        console.log(eventData);
        logEvent('onErrorOccurred', eventData, tabId);
